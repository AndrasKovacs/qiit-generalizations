...
-ᶜ displayed modell:
(Γ : Con)ᶜ : Sub Ωᴺ Γᴺ → Γᴬ
...

-ʳ displayed modell:
(Γ : Con)ʳ : (ν : Sub Ωᴺ Γᴺ) → Γᴹ (F ν (Ωᶜ id)) (F ν ω)

Itt a "Γᴹ (F ν (Ωᶜ id)) (F ν ω)" jól típusozott a következő miatt:

- F ν : F Ωᴺ → F Γᴺ
- Γᴹ  : Γᴬ → Γᴬ → Set
- Az -ᴬ       morfizmus ToSSyn-ből SET-be megy
- Az (F ∘ -ᴺ) morfizmus szintén ToSSyn-ből SET-be megy
- Tehát az inicialitás miatt a kettő egyenlő, azaz F Ωᴺ = Ωᴬ és F Γᴺ = Γᴬ
