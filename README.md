...
- Consider signatures which are not generative and does not have initial algebras, but still have flCwFs of algebras. For example, fixing two categories C, D in the external theory, the signature for functors [C, D] is such, because it has a function which returns in an external set instead of an inductively defined set.
