...
\begin{alignat*}{3}
  & \Con_{\bA} && :\equiv \Set_{\max(i+1, j)} \\
  & \Sub_{\bA}\,\Gamma\,\Delta && :\equiv \Gamma \ra \Delta\\
  & \Ty_{\bA}\,\Gamma && :\equiv \Gamma \ra \Set_{\max(i+1, j)}\\
...
types. Hence, we have:
\begin{alignat*}{3}
  & \Con_{\bD}\,\ulGamma && :\equiv \ulGamma \ra \Set_{\max(i+1, j)} \\
  & \Sub_{\bD}\,\Gamma\,\Delta\,\ulsigma && :\equiv (\ulgamma : \ulGamma) \ra \Gamma\,\gamma \ra \Delta(\llb\ulsigma\rrb_{\bA}\,\ulgamma)\\
  & \Ty_{\bD}\,\Gamma\,\ulA && :\equiv (\ulgamma : \ulGamma) \ra \Gamma\,\ulgamma \ra \llb\ulA\rrb_{\bA}\,\ulgamma \ra \Set_{\max(i+1, j)}\\
...
computed as $\llb\Gamma\rrb_{\bS}\,\ulgamma\,\gamma$.
\end{definition}
...
to any higher level. This requires showing that $\Gamma$-algebras are
...
$\ToS_{l, j}$. In the following, we notate the level of algebras computed by
  $\llb\blank\rrb_{\bA}$ as an extra index, as in
...
Kaposi et al's work \cite{kaposi2019constructing} is the direct precursor of our
work. They don not consider infinitary constructors or constructors with
recursive equations, which make their semantics considerably simpler. They also
do not provide a model theory of signatures, instead they assume signatures
as an ad-hoc QIIT.

Streicher's initiality proof \cite{streicher2012semantics} is a direct
construction of a concrete QIIT: that of the syntax of type theory. He uses
preterms, typing relations and quotients to construct the initial model of type
theory in a set-theoretic metalanguage. A recent formalisation of the same
...
Higher inductive types (HITs) are generalisations of QIITs in settings with
