...
\begin{alignat*}{3}
  ...
  & \coe_{\id} && : \coe\,\id\,\Gamma = \Gamma\\
  & \coe_{\circ} && : \coe\,(\ulsigma\circ\uldelta)\,\Gamma = \coe\,\ulsigma\,(\coe\,\uldelta\,\Gamma)\\
  & \coh_{\id} && : \coh\,\id\,\Gamma = \id\\
  & \coh_{\circ} && : \coh\,(\ulsigma\circ\uldelta)\,\Gamma = \coh\,\ulsigma\,(\coe\,\uldelta\,\Gamma)
...
\begin{alignat*}{3}
  ...
  & \coe_{\id} && : \coe\,\id\,A = A\\
  & \coe_{\circ} && : \coe\,\ult\,(\coe\,\uldelta\,A) = \coe\,(\ult\circ\uldelta)\,A\\
  & \coh_{\id} &&: \coh\,\id\,A = \id\\
  & \coh_{\circ} &&: \coh\,(\ult\circ\uldelta)\,A = \coh\,\ult\,(\coe\,\uldelta\,A)
          \circ \coh\,\uldelta\,A
\end{alignat*}
Additionally, for $\sigma : \Sub_{\bA}\,\Gamma\,\Delta\,\ulsigma$, we have
\begin{alignat*}{3}
  & \coe_{[]} &&: \coe\,(\ult[\ulsigma])\,(A[\sigma]) = (\coe\,\ult\,A)[\sigma]\\
  & \coh_{[]} &&: \coh\,(\ult[\ulsigma\circ \p,\q])\,(A[\sigma]) = (\coh\,\ult\,A)[\sigma]
\end{alignat*}
...
and $\boldsymbol{A[\sigma\circ\delta]} = \boldsymbol{A[\sigma][\delta]}$. This
is rather laborious to fully write out, so we omit most details and focus on the
main points.

The underlying sets are given by simple composition:
\begin{alignat*}{3}
...
$\boldsymbol{A[\sigma\circ\delta]} = \boldsymbol{A[\sigma][\delta]}$, follow
from Lemma \ref{lem:idcomppres} and split cleaving given by $\coe_{\id}$,
$\coe_{\circ}$, $\coh_{\id}$ and $\coh_{\circ}$ in $\bA$.

\subsection{Terms}

$\boldsymbol{\Tm\,\Gamma\,A} : \Set_{1 + \max(i, j)}$ is defined as the type of \emph{weak flCwF sections} of $\bA$.

\begin{definition}[Weak flCwF section] Given $\bA : \bTy\,\bGamma$, the underlying functions of $\bt : \bTm\,\bGamma\,\bA$ are as follows:
\begin{alignat*}{3}
...
Such that
\begin{enumerate}
  \item $\bsigma\,(\ulA[\ulsigma]) = (\bsigma\,\ulA)\,[\bsigma\,\ulsigma]$
  \item $\bsigma\,(\ult[\ulsigma]) = (\bsigma\,\ult)\,[\bsigma\,\ulsigma]$
  \item The unique map $\epsilon_{\bA} : \Sub\,(\bsigma\,\emptycon)\,\emptycon_{\bGamma}\,\id$ has a vertical retraction.
  \item Each $(\bsigma\,\p,\,\bsigma\,\q) : \Sub\,(\bsigma\,(\ulGamma\,\ext\,\ulA))\,(\bsigma\,\ulGamma\,\ext\,\bsigma\,\ulA)\,\id$ has a vertical inverse.
\end{enumerate}
...
$\boldsymbol{\blank[\blank] : \Tm\,\Delta\,A \ra (\sigma : \Sub\,\Gamma\,\Delta)
  \ra \Tm\,\Gamma\,(A[\sigma])}$ is given the similarly to
...
$\bA$ with corresponding base components in $\bGamma$, using metatheoretic
$\Sigma$-types. It is a straightforward extension of total categories in
\cite{displayedcats}.
...
taking a first projection for each component. $\boldsymbol{\q : \Tm\,(\Gamma\ext
  A)\,(A[\p])}$ is likewise a strict flCwF section given by second projections.
