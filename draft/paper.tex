...
\todo{revise metatheory intro, use Timany cumulativity}
\todo{recheck universe levels, prettify levels}
\todo{explain the limitations and design choices in univ levels}
\todo{write the rest}
\todo{see if we can cut back on flCwF, add more examples instead}
...
syntax. Several variants have been used, from contextual categories \cite{TODO}
and comprehension categories \cite{TODO} to categories with families
\cite{TODO}.

Real, surreal \cite{TODO} and ordinal numbers \cite{TODO} are examples of
\emph{infinitary} QIITs, which are represented as inductive trees which may
branch infinitely.
...
The current paper extends previously formalized QIITs
\cite{kaposi2019constructing}. We add the following features:
\begin{enumerate}
  \item
  \textbf{Large constructors, large elimination} and algebras at different
  universe levels. This fills in an important formal gap, since large
  elimination is routinely used in the metatheory of type theories, but it has
  not been but it has not been presented explicitly in previous works about
  QIITs.
  \item
...
of QIITs. We discuss related work and conclude in Sections \ref{TODO}
...
\todo{univ lvl for pi sigma}

%% Both for functions and $\Sigma$, the output universe level is given as the
%% maximum of the levels of the constituent type.
...
relation on types, and a corresponding subtyping rule.
\begin{mathpar}
  \inferrule*{i \leq j}
...
of cETT terms. This is sufficient in this paper, since we do not need to
internalize level polymorphism.

We reuse the notation of cETT functions for
...
For levels $i$ and $j$, $\ToS_{i,j} : \Set_{\max(i,\,1+j)}$ is a cETT type whose
...
A QIIT signature at level $i$ is a context in an arbitrary p$M : \ToS_{i,j}$
...
conditions in various cubical type theories \cite{TODO}. Note that our $\Id$
...
In the following, for $i$ and $j$ levels we define a model $\bM_{i,j} : \ToS_{i,
  1+\max(i,1+j)}$, such than $\Con_{\bM_{i,j}}$ is a type of structured categories (of
...
For each level $i$ we define $\flCwF_i : \Set_{1+i}$ as an iterated $\Sigma$-type
...
For $A : \Set_i$ and $\bB : A \ra \bTy\,\bGamma$, we define $\bPie\,A\,\bB
...
With this, we have defined the $\bM_{i,j} : \ToS_{i, 1+\max(i,1+j)}$ model that we set out to define
...
\begin{definition}[Signature for ToS] For each level $i$,
we define $\ToSSig_i : \Sig_{1+i}$, as the signature for the theory of
signatures with external sets in $\Set_i$. This is a large and infinitary QIIT
signature, as we have $\Pie$ and $\Piinf$ abstracting over $A : \Set_i$ and
...
\begin{alignat*}{3}
  ...
  &           && \hspace{4.2em}(\Pie \Set_i (\lambda\,A.\,(A \arrinf \Ty\,\Gamma)\arri\El(\Ty\,\Gamma))))\\
  &           && ...
\end{alignat*}
\end{definition}

Now, for each $j$, the interpretation of $\ToSSig_i$ in $\bM_{1+i,j}$ yields an
...
internal representation of $\ToS$. Also, since we have an flCwF of
$\ToS_{i,j}$-algebras, we can use Definition \ref{def:induction} for the notion
of $\ToS$-induction.
...
In this section we construct QIITs from initial ToS-algebras. Before we can assume
the existence of such algebras. First, we need cumulative subtyping for $\ToS$.
\todo{foobar}

\begin{lemma}[Cumulativity of $\ToS$] If $j \leq j'$, then $\ToS_{i,j} \leq \ToS_{i, j'}$.
\end{lemma}
\begin{proof}
  The $j$ universe level only appears in function codomains, hence
  the subtyping rules for functions and $\Sigma$ can be used to get
  $\ToS$ subtyping.
\end{proof}

\todo{assumption in environment}

%% For each level $i$,
%% \begin{enumerate}
%%   \item We assume that $\tos_i : \ToS_{i, 1+i}$ exists.
%%   \item For each $i'$ such that $(1+i) \leq i'$, we assume $\Induction
%% \end{enumerate}

%% \textbf{For each $i$, we assume that $\tos_i : \ToS_{i, 1+i}$ exists}. The reason for the
%% $1 + i$ level of $\ToS$ sorts is that $\tos_i$ may contain $A : \Set_i$ types in
%% $\Ty$, and since we intend $\tos_i$ to be a freely generated algebra, the sorts
%% must be large enough to avoid size paradoxes.



%% \textbf{For each $i$ and $j$, we assume $\mathsf{ToSind :

%% \todo{assumption as environment}
