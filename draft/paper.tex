...
\todo{revise cETT (compress), more mention of interp of term constructors in model, include missing iso-cleaving to type subst}
...
In this section we construct QIITs from initial ToS-algebras. From now on, we assume
the existence of such algebras.
...
\begin{lemma}[Cumulativity of $\ToS$] If $i \leq i'$ and $j \leq j'$,
  then $\ToS_{i,j} \leq \ToS_{i', j'}$.
\end{lemma}
\begin{proof} $\ToS_{i,j}$ is uniformly defined in $i$, $j$, hence cumulative subtyping extends
  through dependent pairs and functions to $\ToS$.
\end{proof}
