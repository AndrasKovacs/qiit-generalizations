...
%% is inductive
%% when considered as the element of $\ToS_{j',j}$ by Lemma
%% \ref{lemma:cumulativity}.

%% In the induction assumption, we are relying on the fact that there is an flCwF
%% whose objects are $\ToS_{j',j}$ algebras.

%% Above, $\syn$ abbreviates ``syntax'' for $\ToS$. In the assumption of induction,
%% we are using Definition \ref{def:induction}.
