...
First, we need to describe three model fragments of $\bM$, which can be
respectively used to compute notions of algebras, displayed algebras and
sections for each syntactic signature. This is a rephrasing of the
$\blank^{\A}$, $\blank^{\D}$ and $\blank^{\S}$ interpretations in
...
We omit the rest of the definition of $\bT_{\ulOmega}$. The interpretations of equations in
the CwF and fl-structure parts become fairly technical, and we also need to
utilize iso-cleaving to interpret type substitution and substitution
...
Cartmell \cite{gat}. Displayed categories Ahrens \cite{displayedcats}.

Paolo paper \cite{gabe}. Finitary case \cite{kaposi2019constructing}.

Direct construction of concrete QIIT: Streicher
\cite{streicher2012semantics}, Brunerie \cite{brunerie}, Ambroise
\cite{induction-is-enough}.

Higher inductive types: HIT descriptions \cite{niels,hiit}, HITs in
cubical type theories\cite{cubicalhits,10.1145/3290314}, semantics for
subclasses of HITs
\cite{lumsdaineShulman,10.1145/3209108.3209130,nielsmsc,sojakova,moeneclaey}
Construction of example IITs without UIP: Jasper \cite{jasper}.
