...
\textbf{Inductive function space}, with $\Pii : (a : \Tm\,\Gamma\,\U) \ra
\Ty\,(\Gamma\,\ext\,\El\,a) \ra \Ty\,\Gamma$ and application as $\appi :
\Tm\,\Gamma\,(\Pii\,a\,B)\ra \Tm\,(\Gamma\,\ext\,\El\,a)\,B$ and its
inverse $\lami$.

\textbf{External function space} with $\Set_i$ domain: $\Pie : (A : \Set_i)\ra(A
\ra \Ty\,\Gamma)\ra \Ty\,\Gamma$, with $\appe : \Tm\,\Gamma\,(\Pie\,A\,B)\ra((x
: A)\ra \Tm\,\Gamma\,(B\,x))$ and its inverse $\lame$.

\textbf{Infinitary function space}, with $\Piinf : (A : \Set_i)\ra(A \ra
...
We define $\bCon : \Set_{1+\max(i,j+1)}$ as $\flCwF_{\max(i,j+1)}$. We define
$\flCwF$ as follows.
...
\begin{enumerate}
  \item A CwF with underlying sets all in $\Set_i$. We reuse the notation
    from definition \ref{def:tos}.
...
\begin{definition} A \emph{weak flCwF morphism} $\boldsymbol{\sigma : \Sub\,\Gamma\,\Delta}$ is a functor between underlying categories, which additionally maps types to types and terms to terms, and satisfies the following:
  \begin{enumerate}
...
%% We do not require preservation of fl-structure, because the above conditions already insure that.


\begin{theorem} Every $\boldsymbol{\sigma : \Sub\,\Gamma\,\Delta}$ preserves fl-structure up to type isomorphism. That is, we have
\begin{alignat*}{3}
...
verification that $\bsigma$ preserves term and substitution formers in the
fl-structure.
\end{proof}
...
\begin{lemma} The derived preservation isomorphisms for the fl-structure can be decomposed analogously; all derived isomorphisms in $\bid$ are identities, and we have
\begin{alignat*}{3}
...
\end{alignat*}
Here, $\blank\circ\blank$ denotes composition of type morphisms.
\end{lemma}
...
\begin{definition}[Displayed flCwF] The type of displayed flCwFs is given as the logical predicate interpretation \cite{TODO} of $\flCwF_i$. For each flCwF component in $\bGamma$, there is a component in a displayed flCwF which ``lies over'' it.
...
Concretely, $\boldsymbol{A : \Ty\,\Gamma}$ has the following underlying sets, which we call displayed contexts, substitutions, types and terms respectively.
\begin{alignat*}{3}
...
\begin{definition} A \emph{vertical morphism} lies over an identity morphism. We use this definition for context and type morphisms as well.
\end{definition}
...
\begin{definition}[Context iso-cleaving] This lifts a base context isomorphism to a displyed one. It consists of
\begin{alignat*}{3}
...
\begin{alignat*}{3}
  & \coe\,\id\,\Gamma && = \Gamma\\
  & \coe\,(\ulsigma\circ\uldelta)\,\Gamma && = \coe\,\ulsigma\,(\coe\,\uldelta\,\Gamma)\\
  & \coh\,\id\,\Gamma && = \id\\
  & \coh\,(\ulsigma\circ\uldelta)\,\Gamma && = \coh\,\ulsigma\,(\coe\,\uldelta\,\Gamma)
...
\begin{alignat*}{3}
  & \coe\,\id\,A && = A\\
  & \coe\,\ult\,(\coe\,\uldelta\,A) && = \coe\,(\ult\circ\uldelta)\,A\\
  & \coh\,\id\,A && = \id\\
  & \coh\,(\ult\circ\uldelta)\,A && = \coh\,\ult\,(\coe\,\uldelta\,A)
          \circ \coh\,\uldelta\,A
\end{alignat*}
Also, whenever $\sigma : \Sub_{\bA}\,\Gamma\,\Delta\,\ulsigma$, we have
\begin{alignat*}{3}
  & \coe\,(\ult[\ulsigma])\,(A[\sigma]) && = (\coe\,\ult\,A)[\sigma]\\
  & \coh\,(\ult[\ulsigma\circ \p,\q])\,(A[\sigma]) && = (\coh\,\ult\,A)[\sigma]
\end{alignat*}
...
We seek to define $\boldsymbol{\blank[\blank] : \Ty\,\Delta \ra \Sub\,\Gamma\,\Delta \ra \Ty\,\Gamma}$.
