...
%% Whereas the initial category can be given by first defining the type of objects
%% (the empty type) and then defining morphisms separately, for an initial model of
%% a type theory given as a CwF, one has to define $\mi{Ob}$, $\mi{Mor}$ and two
%% extra sorts $\mi{Ty}$ and $\mi{Tm}$ \emph{inductive-inductively}, at the same
%% time. This is illustrated by the following excerpt of the signature for CwFs
%% extending the previous signature for categories.
%% \begin{alignat*}{3}
%%   & \mi{Ty}        && : \mi{Ob}\ra \Set \\
%%   & \mi{U}         && : \mi{Ty\,i} \\
%%   & \mi{empty}     && : \mi{Ob} \\
%%   & \mi{ext}       && : (I:\mi{Ob})\ra \mi{Ty}\,I\ra \mi{Ob} \\
%%   & \blank[\blank] && : \mi{Ty}\,J\ra \mi{Mor}\,I\,J\ra \mi{Ty}\,I
%% \end{alignat*}
%% While signatures for type theories usually only contain finitary constructors,
%% some QIITs have infinitary constructors, such as the following signature for
%% countably branching trees where the order of the subtrees does not matter,
%% i.e.\ permuting subtrees yields an equal tree.
%% \begin{alignat*}{3}
%%   & \mi{Tree} && : \U \\
%%   & \mi{leaf}  && : \mi{Tree} \\
%%   & \mi{node} && : (\N\ra \mi{Tree})\ra \mi{Tree} \\
%%   & \mi{perm} && : (g :\N\ra \mi{Tree})(f : \N\ra\N)\ra \mathsf{isIso}\,f\ra \\
%%   & && \hspace{0.8em} \mi{node}\,g = \mi{node}\,(g\circ f)
%% \end{alignat*}
%% Additional examples for infinitary QIITs are real, surreal numbers
%% \cite{HoTTbook}, ordinal numbers \cite{lumsdaineShulman} and the partiality
%% monad \cite{partiality}.

%% \subsection{Contributions}
%% The current paper extends the syntax and semantics of QIITs as previously described
%% in the literature \cite{kaposi2019constructing, qiits, dijkstra2017quotient}. We add the following features:
...
\begin{enumerate}
  ...
  theories, as boundary conditions \cite{cchm, angiuli2016computational, angiuli2018cartesian}.
\end{enumerate}
...
conditions in various cubical type theories \cite{cchm, angiuli2016computational, angiuli2018cartesian}. Note that our $\Id$
...
%% For some $(N,\,z,\,s)$ algebra,
%% $\Ty\,(N,\,z,\,s)$ is defined as $(N^P : N \to \Set)\times N^P\,z \times ((n :
%% N)\to N^P\,n \to N^P(s\,n))$, the type of displayed algebras, and
%% $\Tm\,(N,\,z,\,s)\,(N^P,\,z^P,\,s^P)$ is $(N^S : (n : N)\to N^P\,n) \times (N^S\,z = z^P)
%% \times ((n : N) \to N^S\,(s\,n) = s^P\,(N^S\,n))$.

%% Here, $\Con_{\bGamma}$ is the type of algebras for some signature, e.g.\ the
%% type of natural number algebras in the case of
%% $\mathsf{NatSig}$. $\Ty_{\bGamma}\,\Gamma$ is a displayed algebra over a
%% concrete algebra
...
equivalence, external to cETT (because of unvierse polymorphism), and not an
isomorphism.
...
type-theoretic syntax. Compared to our QII signatures, his signatures support
sort equations but do not cover infinitary constructors or recursive
equations. A way to encode sort equations in our signatures is using
...
equivalence of initiality and induction, but do not construct initial algebras,
nor would that be possible because of the lack of a positivity restriction.
