...
\begin{alignat*}{3}
  ...
  & idr && : f\circ id = f
\end{alignat*}
The sort $Mor$ is indexed twice over the sort $Ob$, $ass$, $idl$ and
$idr$ express equations between the constructors $\blank\circ\blank$
and $id$.
...
\begin{alignat*}{3}
  & Ty && : Ob\ra \U \\
  & empty  && : Ob \\
  & ext && : (\Gamma:Ob)\ra Ty\,\Gamma\ra Ob \\
  & \blank[\blank] && : Ty\,\Delta\ra Mor\,\Gamma\,\Delta\ra Ty\,\Gamma
\end{alignat*}

While signatures for type theory usually only contain finitary
constructors, important examples of QIITs have infinitary
constructors, such as the following signature for infinitaly branching
trees where the order of subtrees does not matter.
\begin{alignat*}{3}
  & Tree && : \U \\
  & leaf  && : Tree \\
...
\begin{definition}[The $\Set$ model of $\ToS$] For each $j$, we have $\bA : \ToS_{j+1, j}$, which can be given by restricting the $\bM$ model
...
\begin{alignat*}{3}
  & \Con_{\bA} && :\equiv \Set_{j+2} \\
  & \Sub_{\bA}\,\Gamma\,\Delta && :\equiv \Gamma \ra \Delta\\
  & \Ty_{\bA}\,\Gamma && :\equiv \Gamma \ra \Set_{j+2}\\
  & \Tm_{\bA}\,\Gamma\,A && :\equiv (\gamma : \Gamma) \ra A\,\gamma
\end{alignat*}
Now, for $\Gamma : \Con_{\syn}$, the type of $\Gamma$-algebras is given by
$\llb\Gamma\rrb_{\bA}$. E.g.\, $\llb\NatSig\rrb_{\bA}$ yields a left-nested
$\Sigma$-type of pointed sets with an endofunction. Also, $\llb\Gamma\rrb_{\bM}$
extends $\llb\Gamma\rrb_{\bA}$ to an flCwF of $\Gamma$-algebras.
\end{definition}

\begin{definition}[Logical predicate model of $\ToS$ over the $\Set$ model]
For each $j$ level we have $\bD$, which is a displayed $\ToS$ model over $\bA$
at level $j$. This model, analogously to $\bA$, is given by restricting $\bM$ to
the \emph{third} components everywhere, corresponding to types or actions on
types. Hence, we have:
\begin{alignat*}{3}
  & \Con_{\bD}\,\ulGamma && :\equiv \ulGamma \ra \Set_{j+2} \\
  & \Sub_{\bD}\,\Gamma\,\Delta\,\ulsigma && :\equiv (\ulgamma : \ulGamma) \ra \Gamma\,\gamma \ra \Delta(\llb\ulsigma\rrb_{\bA}\,\ulgamma)\\
  & \Ty_{\bD}\,\Gamma\,\ulA && :\equiv (\ulgamma : \ulGamma) \ra \Gamma\,\ulgamma \ra \llb\ulA\rrb_{\bA}\,\ulgamma \ra \Set_{j+2}\\
  & \Tm_{\bD}\,\Gamma\,A\,\ult && :\equiv (\ulgamma : \ulGamma)(\gamma : \Gamma\,\ulGamma)
    \ra A\,\ulgamma\,\gamma\,(\llb\ult\rrb_{\bA}\,\ulgamma)
\end{alignat*}
\end{definition}

For $\Gamma : \SynSig_j$, the type of displayed $\Gamma$-algebras over some
$\ulgamma : \llb\Gamma\rrb_{\bA}$ is given by $\llb\Gamma\rrb_{\bD}\,\ulgamma$. In
other words, $\llb\Gamma\rrb_{\bD}$ yields the notion of types in the flCwF of
$\Gamma$-algebras given by $\llb\Gamma\rrb_{\bM}$.

\begin{definition}[Displayed algebra section model of $\ToS$] Analogously to
$\bA$ and $\bD$, we define $\bS$ as a displayed $\ToS$ model over the total
model of $\bD$, which is given by restricting $\bM$ to the fourth components,
corresponding to interpretations of terms and actions on terms.

For $\ulgamma : \llb\Gamma\rrb_{\bA}$ and $\gamma : \llb\Gamma\rrb_{\bD}\,\ulgamma$,
the type of $\Gamma$-sections is computed as $\llb\Gamma\rrb_{\bS}\,\ulgamma\,\gamma$.
\end{definition}
...
algebra for $\NatSig : \SynSig_0$ has an underlying set in $\Set_{j+1}$. This is
a bit inconvenient, since normally we would have natural number in $\Set_0$. Our
current term model construction cannot avoid this level bump, since $\syn_j$ is
necessarily a large type, and we do not have a way to construct a small set from
...
%% We assume $\ulOmega : \SynSig_j$.
%% We abbreviate the term algebra
%% $\llb\ulOmega\rrb_{\bT_{\ulOmega}}\,\id$ as

%% $\llb\ulOmega\rrb_{\bT_{\ulOmega}}\,\id$ as b
%% $\llb\ulOmega\rrb_{\bT_{\ulOmega}}\,\id$ as
...
Cartmell \cite{gat} defines generalised algebraic theories (GATs)
using type theoretic syntax. Compared to our QII signatures, his
signatures support sort equations but do not cover infinitary
constructors or recursive equations. A way to encode sort equations in
our signatures is using isomorphisms instead of equalities. It seems
that infinitary constructors and sort equations are not compatible in
a setting with UIP. In contrast to our algebraic definition,
Cartmell's signatures are given by presyntax, named variables and
typing relations. He also does not construct categories of algebras
explicitly, but is concerned with showing that the classifying
category for GATs is that of contextual categories.

A more semantic approach to QIITs is given by Altenkirch et al.\
\cite{gabe}. They generalise the usual functorial semantics of
...
Kaposi et al's work \cite{kaposi2019constructing} is the most direct
precursor of our work. They don't consider infinitary constructors or
constructors with recursive equations which make their semantics much
simpler.

Streicher's initiality proof \cite{streicher2012semantics} is a direct
construction of a concrete QIIT: that of the syntax of type theory. He
uses preterms, typing relations and quotients to construct the initial
model of type theory in a set-theoretic metalanguage. A recent
formalisation of the same result for a larger type theory was done by
Brunerie et al.\ \cite{brunerie} in Agda with UIP, function
extensionality and quotients. Lafont et al.\
\cite{induction-is-enough} construct all finitary inductive-inductive
types in Agda using only UIP and function extensionality. Examples of
inductive-inductive types are constructed without using UIP by Hugunin
\cite{jasper}.

Higher inductive types (HITs) are generalisations of QIITs in settings
with proof-relevant identity types, in fact, they were introduced
before QIITs \cite{HoTTbook}. \cite{niels} is a description of a
subset of HITs. \cite{hiit} describes a syntax for higher
inductive-inductive types using a theory of signatures similar to ours,
however does not construct categories of algebras or show
initiality. Semantics for different subclasses of HITs are given by
...
Our notion of displayed CwF is an extension of displayed category
\cite{displayedcats} however in a setting with UIP.
