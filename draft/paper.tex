...
recover ToS algebras from $\ToSSig$. This follows by computation of the
interpretation and the fact that $\ToSSig$ is the internal representation of
$\ToS$. Also, since we have an flCwF of $\ToS_{i,j}$-algebras, we can use
Definition \ref{def:induction} for the notion of $\ToS$-induction.
...
%% \begin{theorem}  For each $j$, the interpretation of $\ToSSig_i$ in $\bM_{1+i,j}$
%% yields an flCwF $\bGamma$ such that $\Con_{\bGamma} = \ToS_{i,j}$. In short, we
%% can recover ToS algebras from $\ToSSig$.
%% \end{theorem}
%% \begin{proof}
%% \end{proof}


%% Overall, this is just rephrasing Definition \ref{def:tos}
%% internally to an arbitrary $M : \ToS_{1+i,j}$; recall from Definition
%% \ref{def:signature} that $\Sig_{1+i} = (j : \mathbb{N})(M : \ToS_{1+i,j})\ra
%% p\Con_M$. For example, the underlying sets are specified as
