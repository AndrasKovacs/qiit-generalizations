...
time. This is because there is a constructor of $\mi{Ob}$ which refers to
$\mi{Ty}$, constructors of $\mi{Ty}$ refer to $\mi{Ob}$ and $\mi{Sub}$, and so
on. This is illustrated by the following excerpt of the signature for CwFs
...
countably branching trees where the order of subtrees does not matter,
i.e.\ trees with permuted subtrees in a branch are equal.
\begin{alignat*}{3}
  & \mi{Tree} && : \U \\
  & \mi{leaf}  && : \mi{Tree} \\
...
\cite{HoTTbook}. \cite{hitsinprogramming} is a description of a subset of HITs. \cite{hiit}
