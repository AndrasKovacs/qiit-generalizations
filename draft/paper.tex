...
In the following, for $i$ and $j$ levels we define a model $\bM_{i,j} : \ToS_{i,
  \max(i,j+1)+1}$, such than $\Con_{\bM_{i,j}}$ is a type of structured categories (of
...
algebras merely contain elements of $A : \Set_i$ types, while inductive sets are
themselves elements of $\Set_j$. For example, $NatAlg_j : \Set_{\max(0, j+1)} :
\Set_{\max(0, j+1)+1}$,
...
We define $\bCon : \Set_{\max(i,j+1)+1}$ as $\flCwF_{\max(i,j+1)}$.
%% We define
%% $\flCwF$ as follows.
